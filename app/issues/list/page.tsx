...
const IssuesPage = async ({ searchParams }: Props) => {
  ...
  const columns: { 
    ...
    { label: 'Staus', value: 'status', className: 'hidden md:table-cell'},
          ...
          <Table.Row>
            { columns.map((column) => ( 
              <Table.ColumnHeaderCell key={column.value}>
                <NextLink href={{
                  query: { ...searchParamsAwait, orderBy: column.value }
                }}>{column.label}</NextLink>
                {column.value === searchParamsAwait.orderBy && <ArrowUpIcon className='inline'/>}
